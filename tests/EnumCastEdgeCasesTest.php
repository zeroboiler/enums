...
describe('EnumCast Edge Cases', function (): void {
    ...
    describe('set() edge cases', function (): void {
        ...
        it('throws InvalidArgumentException for BackedEnum instance from different enum class in set()', function (): void {
            $cast = new EnumCast(UserStatus::class);

            // Passing a Priority enum (different enum class) to a UserStatus cast
            // should throw — prevents silent data corruption (fixed in issue #20)
            expect(fn (): mixed => $cast->set(
                model: new class {},
                key: 'status',
                value: Priority::HIGH,
